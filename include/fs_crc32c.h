...
inline uint32 crc32c_hw_x86(uint32 seed, const uint8 *p, size_t n) {
    ...
    const uint64 *q = reinterpret_cast<const uint64 *>(p);
    while (n >= 8) {
        c  = _mm_crc32_u64(c, *q++);
        n -= 8;
    }
    p          = reinterpret_cast<const uint8 *>(q);
    uint32 c32 = static_cast<uint32>(c);
    while (n--) {
...
inline uint32 crc32c(const void *data, size_t len, uint32 seed = 0xFFFFFFFFu) {
    const uint8 *p = static_cast<const uint8_t *>(data);
#if defined(__SSE4_2__) && (defined(__x86_64__) || defined(_M_X64))
    return crc32c_hw_x86(seed, p, len);
#elif defined(__aarch64__)
    return crc32c_hw_arm(seed, p, len);
#else
    return crc32c_sw(seed, p, len);
#endif
}
